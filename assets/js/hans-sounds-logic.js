(function () {
  const data = window.hansSoundsData;

  const hansOpeners = [
    "Sehr gut 🎧",
    "Ja, genau 👌",
    "Gut, wir schauen das an 😄",
    "Klar 👍",
    "Das ist wichtig für Aussprache"
  ];
  ...
  function initHansSounds() {
    ...
    if (!box.dataset.initialized) {
      ...
      box.dataset.lesson = "sounds";
    }
  }

  window.askHansChat = askHansChat;
  window.sendHansMessage = sendHansMessage;
  window.initHansSounds = initHansSounds;

  document.addEventListener("DOMContentLoaded", initHansSounds);
})();