(function () {
  const data = window.hansPlanData;
  ...
  function initHansPlan() {
    ...
    if (!box.dataset.initialized) {
      ...
      box.dataset.lesson = "plan";
    }
  }

  window.askHansChat = askHansChat;
  window.sendHansMessage = sendHansMessage;
  window.initHansPlan = initHansPlan;

  document.addEventListener("DOMContentLoaded", initHansPlan);
})();